...
function startNOAACronJob() {
    ...
    cron.schedule('04 21 * * *', async () => {
        ...
        const currentMonth = `${today.getFullYear()}${String(today.getMonth() + 1).padStart(2, '0')}`; // e.g. '202506'
        ...
        try {
            const { tempFilename, precipFilename, tempDate, precipDate } = await getLatestNOAAFileLinks();
            ...
            try {
                ...
                await axios.get(`http://localhost:${port}/addallcountydata`);
